package com.webtoonchat.toonchat.security.jwt.exception;
...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
	...
	public void commence(
		...
		} else if (exception.equals(JwtExceptionCode.INVALID_TOKEN.getCode())) { //잘못된 토큰인 경우
			log.error("entry point >> invalid token");
			setResponse(response, JwtExceptionCode.INVALID_TOKEN);
		} else if (exception.equals(JwtExceptionCode.EXPIRED_TOKEN.getCode())) { //토큰 만료된 경우
			log.error("entry point >> expired token");
			setResponse(response, JwtExceptionCode.EXPIRED_TOKEN);
		} else if (exception.equals(JwtExceptionCode.UNSUPPORTED_TOKEN.getCode())) { //지원되지 않는 토큰인 경우
